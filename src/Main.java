...
public class Main implements GLEventListener, KeyListener, MouseListener, MouseMotionListener {
	...
	public void display(GLAutoDrawable arg0) {
		...
		gl.glLineWidth(2.0f);
		//gl.glPointSize(1.0f);
		gl.glBegin(GL.GL_LINE_LOOP);//circulo da direita
			for (int i = 0; i < 360; i += 5) {
				gl.glVertex2d(RetornaX(i, 100) + 100, RetornaY(i, 100) + 100);
	
			}
		gl.glEnd();
		gl.glBegin(GL.GL_LINE_LOOP);//circulo da esquerda
			for (int i = 0; i < 360; i += 5) {
				gl.glVertex2d(RetornaX(i, 100) - 100, RetornaY(i, 100) + 100);
	
			}
		gl.glEnd();
		gl.glBegin(GL.GL_LINE_LOOP);//circulo de baixo
			for (int i = 0; i < 360; i += 5) {
				gl.glVertex2d(RetornaX(i, 100), RetornaY(i, 100) - 100);
	
			}
		gl.glEnd();
		gl.glColor3f(0.0f, 254.0f, 255.0f);
		gl.glLineWidth(1.0f);
		gl.glBegin(GL.GL_LINE_LOOP);//triangulo
				gl.glVertex2d(0, -100);
